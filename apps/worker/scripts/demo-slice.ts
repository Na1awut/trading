...
import { addToWatchlist, createPrismaClient, findOrCreateUser, listSignalEvents } from '@signals/db';
...
async function main() {
  ...
  const user = await findOrCreateUser(prisma, { firebaseUid: 'dev:demo@example.com', email: 'demo@example.com' });
  ...
  if (!hasDevice) {
    await prisma.device.create({
      data: { userId: user.id, token: 'demo-console-device-token', platform: 'android', provider: 'FCM' },
    });
  }

  step(2, `Market data: NVDA ${timeframe} candles where the last completed candle is an EMA 9/21 bullish cross`);
  const now = Date.now();
  ...
  await prisma.signalState.deleteMany({ where: { signalDefinition: { ticker: 'NVDA', timeframe } } });

  step(3, 'Worker evaluates signals (completed candles only)');
  const notifier = createNotificationSender('console', logger);
  const run = () =>
    runEvaluationCycle({ prisma, marketData: provider, notifier, logger, ingest: false, now: () => now });
  ...
  console.info('or: curl -H "Authorization: Bearer dev:demo@example.com" localhost:4000/signal-events');
}
