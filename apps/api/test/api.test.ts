...
describe('watchlist', () => {
  it('adds, lists with quotes, and removes tickers', async () => {
    ...
    expect(items[0].quote).toMatchObject({
      symbol: 'NVDA',
      source: 'mock',
    ...
    expect(
      (await app.inject({ method: 'DELETE', url: '/watchlist/NVDA', headers: auth() })).statusCode,
    ).toBe(404);
...
describe('assets', () => {
  it('searches symbols', async () => {
    ...
    expect(
      (await app.inject({ method: 'GET', url: '/assets/search?q=', headers: auth() })).statusCode,
    ).toBe(400);
  });
  ...
  it('404s unknown assets', async () => {
    expect(
      (await app.inject({ method: 'GET', url: '/assets/NOPE', headers: auth() })).statusCode,
    ).toBe(404);
  });
});
...
describe('signals', () => {
  it('subscribes new watchlist tickers to presets (default set enabled)', async () => {
    ...
    const signals = res.json().signals as {
      name: string;
      enabled: boolean;
      isPreset: boolean;
      timeframe: string;
  ...
  it('toggles presets but refuses to edit or delete them', async () => {
    await addTicker('NVDA');
    const [first] = (await app.inject({ method: 'GET', url: '/signals', headers: auth() })).json()
      .signals;
    const off = await app.inject({
      method: 'PATCH',
      url: `/signals/${first.id}`,
      headers: auth(),
    ...
    const edit = await app.inject({
      ...
      url: `/signals/${first.id}`,
      headers: auth(),
      payload: { parameters: { fast: 5, slow: 10 } },
    });
    expect(edit.statusCode).toBe(409);
    expect(
      (await app.inject({ method: 'DELETE', url: `/signals/${first.id}`, headers: auth() }))
        .statusCode,
    ).toBe(409);
  });
  ...
  it('creates, edits and deletes custom signals with validated parameters', async () => {
    ...
    const missing = await app.inject({
      ...
      url: '/signals',
      headers: auth(),
      payload: { ticker: 'NVDA', signalType: 'PRICE_BELOW' },
    });
    ...
    const edited = await app.inject({
      ...
      url: `/signals/${id}`,
      headers: auth(),
      payload: { parameters: { threshold: 200 } },
    });
    expect(edited.json()).toMatchObject({
      name: 'Price above $200.00',
    ...
    expect(
      (await app.inject({ method: 'DELETE', url: `/signals/${id}`, headers: auth() })).statusCode,
    ).toBe(204);
    expect(
      (await app.inject({ method: 'GET', url: '/signals', headers: auth() })).json().signals,
    ).toEqual([]);
  });
...
describe('signal events', () => {
  async function seedEvents(email: string, n: number) {
    ...
    const def = await prisma.signalDefinition.findFirstOrThrow({
      where: { signalType: 'EMA_BULLISH_CROSS', ticker: 'NVDA' },
    });
    for (let i = 0; i < n; i++) {
  ...
  it('lists history newest first with cursor pagination and user isolation', async () => {
    ...
    const page2 = await app.inject({
      method: 'GET',
      url: `/signal-events?limit=3&before=${body1.nextCursor}`,
      headers: auth(),
    });
    ...
    const one = await app.inject({
      method: 'GET',
      url: `/signal-events/${body1.events[0].id}`,
      headers: auth(),
    });
  ...
  it('keeps history when the ticker is removed and shows recent events on the asset', async () => {
    ...
    expect(
      (await app.inject({ method: 'GET', url: '/signal-events', headers: auth() })).json().events,
    ).toHaveLength(2);
  });
});
...
describe('devices & settings', () => {
  it('registers devices, moves re-registered tokens, and sends a test push', async () => {
    ...
    expect(
      (await app.inject({ method: 'POST', url: '/devices/test', headers: auth() })).json().devices,
    ).toBe(0);
